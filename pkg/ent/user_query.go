...
func (uq *UserQuery) QueryPersonals() *PersonalQuery {
	query := (&PersonalClient{config: uq.config}).Query()
...
func (uq *UserQuery) QueryCompanys() *CompanyQuery {
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withPersonals: uq.withPersonals.Clone(),
		withCompanys:  uq.withCompanys.Clone(),
...
// WithPersonals tells the query-builder to eager-load the nodes that are connected to
// the "personals" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithPersonals(opts ...func(*PersonalQuery)) *UserQuery {
	query := (&PersonalClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withPersonals = query
	return uq
}

// WithCompanys tells the query-builder to eager-load the nodes that are connected to
// the "companys" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithCompanys(opts ...func(*CompanyQuery)) *UserQuery {
	query := (&CompanyClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withCompanys = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPersonals; query != nil {
		if err := uq.loadPersonals(ctx, query, nodes,
			func(n *User) { n.Edges.Personals = []*Personal{} },
			func(n *User, e *Personal) { n.Edges.Personals = append(n.Edges.Personals, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withCompanys; query != nil {
		if err := uq.loadCompanys(ctx, query, nodes,
			func(n *User) { n.Edges.Companys = []*Company{} },
			func(n *User, e *Company) { n.Edges.Companys = append(n.Edges.Companys, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadPersonals(ctx context.Context, query *PersonalQuery, nodes []*User, init func(*User), assign func(*User, *Personal)) error {
	...
	query.Where(predicate.Personal(func(s *sql.Selector) {
		s.Where(sql.InValues(user.PersonalsColumn, fks...))
	}))
...
func (uq *UserQuery) loadCompanys(ctx context.Context, query *CompanyQuery, nodes []*User, init func(*User), assign func(*User, *Company)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Company(func(s *sql.Selector) {
		s.Where(sql.InValues(user.CompanysColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
